...
- **THEN** sessions SHALL appear under a collapsible group whose header shows the working directory, and each group SHALL contain only sessions whose `cwd` matches the group header
...
The system SHALL render Copilot sessions in the left pane as a single flat list ordered by `created_at` descending.

#### Scenario: Copilot sessions render flat
- **WHEN** the Copilot session list renders
- **THEN** sessions SHALL appear as a single flat list without grouping, ordered by `workspace.yaml.created_at` descending
...
- **THEN** the row SHALL show the encoded-cwd folder name, a short session id (first 8 characters of `sessionId`), and the session file's modification time
