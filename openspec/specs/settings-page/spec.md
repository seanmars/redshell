...
Provide a tabbed Settings page reachable from the sidebar footer that consolidates Agents and Marketplaces management into a single configuration surface. The active tab is synchronized with a `tab` query parameter for deep linking, and the previously top-level `/agents` (also accepting legacy `/providers`) and `/marketplaces` routes redirect into the corresponding Settings tabs so existing bookmarks continue to work.
...
### Requirement: Settings page renders Agents and Marketplaces as tabs
The system SHALL render the Settings page as a tabbed layout containing at least an Agents tab and a Marketplaces tab, with the active tab rendering the corresponding configuration surface inside the Settings page.
...
#### Scenario: Agents tab shows the agent configuration surface
- **WHEN** the Agents tab is active
- **THEN** the tab content area SHALL display the list of supported AI agents with their configured status, equivalent to what the standalone Agents page previously displayed
...
The system SHALL preserve the previously exposed `/agents` (and the legacy `/providers`) and `/marketplaces` routes as redirects into the Settings page so that existing bookmarks and references continue to work.

#### Scenario: Agents route redirects
- **WHEN** the user navigates to `/agents`
- **THEN** the router SHALL redirect to `/settings?tab=agents` and the Agents tab SHALL be active

#### Scenario: Legacy providers route redirects to agents tab
